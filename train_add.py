...
def train_model(
    ...
    #                     lr=learning_rate, 
    #                     weight_decay=weight_decay,  # 增加权重衰减
    ...
    for epoch in range(1, epochs + 1):
        ...
        if save_checkpoint:
            ...
            # 为注意力模型和普通模型使用不同的文件名
            if hasattr(model, 'attention_type'):  # 检查是否是注意力模型
                filename = f'attention_{model.attention_type}_epoch{epoch}.pth'
                logging.info(f'Attention model checkpoint (type: {model.attention_type}) {epoch} saved!')
            else:
                filename = f'checkpoint_epoch{epoch}.pth'
                logging.info(f'Standard model checkpoint {epoch} saved!')
                
            torch.save(state_dict, str(dir_checkpoint / filename))
...
if __name__ == '__main__':
    ...
    # 这里根据你的数据进行更改
    # n_channels=3表示RGB图像
    # n_classes是你想要每个像素获得的概率数量
    
    # Select model type based on args
    if args.attention:
        logging.info(f'Using Attention U-Net with {args.attention_type} attention mechanism')
        model = AttentionUNet(n_channels=3, n_classes=args.classes, bilinear=args.bilinear, attention_type=args.attention_type)
    else:
